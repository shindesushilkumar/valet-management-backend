...
The system SHALL allow an authenticated user to register a new car by providing a car number. The system MUST associate the car with the authenticated user and return the created car record with its ID.

#### Scenario: Successful car registration
- **WHEN** an authenticated user sends `POST /cars` with a valid car number
- **THEN** the system creates a car record linked to the user and returns HTTP 201 with the car ID, car number, and user ID
...
- **THEN** the system returns HTTP 200 with an array of the user's car records
...
The system SHALL allow an authenticated user to update their car's car number. The system MUST only allow updates to cars owned by the requesting user.

#### Scenario: Successful car update
- **WHEN** an authenticated user sends `PATCH /cars/:id` with a valid car number for their car
